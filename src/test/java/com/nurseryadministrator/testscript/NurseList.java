...
public class NurseList extends SetUp{
	...
	public void initialize()
	{
		try
		{
			...
			nurselistObj = new NurseListObject(driver);
	...
	public void viewNurseDetails()
	{
		try
		{
			log.info("Entered viewNurseDetails test");		
			eTest = eReports.createTest("viewNurseDetails");
			eTest.assignCategory("Employee - List Nurse");
			...
			roleData[3] = (language.equals("Arabic"))?"إختيار الكل":"All";
			expectedArraylist.add(roleData[3]);
			nurselistObj.filterNurse(fullname);
			Thread.sleep(2000);
			autoScrollandClick(nurselistObj.viewButton);
	...
	public void deactivateNurse()
	{
		try
		{
			log.info("Entered deactivateNurse test");		
			eTest = eReports.createTest("deactivateNurse");
			eTest.assignCategory("Employee - List Nurse");
	...
	public void activateNurse()
	{
		try
		{
			log.info("Entered activateNurse test");		
			eTest = eReports.createTest("activateNurse");
			eTest.assignCategory("Employee - List Nurse");
	...
	public void filterByName() 
	{
		try
		{
			...
			employeeData = addemployeedataObj.getEmployeeDetails();
			String fullname = employeeData[0]+" "+employeeData[1]+" "+employeeData[2];
			waitForElementToLoad(nurselistObj.name);
			nurselistObj.filterNurse(fullname);
			Thread.sleep(2000);

			waitForElementToLoad(nurselistObj.totalRecords);
			System.out.println(nurselistObj.totalRecords.getText());
			String countstring=nurselistObj.totalRecords.getText().replaceAll("[^\\d]", "").trim();
			...
			{
				condition = isElementPresent(nurselistObj.lastpagecheck);
				for(int i =1;i<=10;i++)
				{
					if(c+i<=count)
					{
				...
				if(!isElementPresent(nurselistObj.lastpagecheck))
				{
					waitIfElementClickIsIntercepted(nurselistObj.goToNextPage, "click", "");
				}
				c=c+10;
			}
			waitForElementToLoad(nurselistObj.resetButton);
			autoScrollandClick(nurselistObj.resetButton);
			waitIfElementClickIsIntercepted(dashboardObj.employees, "click", "");
		}
		catch(Exception e) {
			e.printStackTrace();
	...
	@Test(priority=5)
	public void editAndViewNurseDetails()
	{
		try
		{
			log.info("Entered editAndViewNurseDetails test");		
			eTest = eReports.createTest("editAndViewNurseDetails");
			eTest.assignCategory("Employee - List Nurse");
			...
			nurselistObj.filterNurse(fullname);
			Thread.sleep(2000);
			autoScrollandClick(nurselistObj.editButton);
			...
			autoScrollandClick(nurselistObj.saveButton);
			waitForElementToLoad(nurselistObj.updateSucessMsg);
			actualArraylist.add(nurselistObj.updateSucessMsg.getText());
			waitForElementToLoad(nurselistObj.resetButton);
			nurselistObj.resetButton.click();
			Thread.sleep(1500);
			nurselistObj.filterNurse(fullname);
			Thread.sleep(2000);
			autoScrollandClick(nurselistObj.viewButton);
	...
	public void verifySorForName()
	{
		try
		{
			...
			waitIfElementClickIsIntercepted(dashboardObj.employees, "click", "");
			dashboardObj.nurseList.click();
			expectedMsg = verifySortButton(nurselistObj.nameSortButton, nurselistObj.sortedName);
			actualMsg = nurselistObj.sortedName.getText();
			waitIfElementClickIsIntercepted(dashboardObj.employees, "click", "");
		}
		catch(Exception e) {
			e.printStackTrace();
	...
	public void verifySorForPhone()
	{
		try
		{
			...
			waitIfElementClickIsIntercepted(dashboardObj.employees, "click", "");
			dashboardObj.nurseList.click();
			expectedMsg = verifySortButton(nurselistObj.phoneSortButton, nurselistObj.sortedPhone);
			actualMsg = nurselistObj.sortedPhone.getText();
			waitIfElementClickIsIntercepted(dashboardObj.employees, "click", "");
		}
		catch(Exception e) {
			e.printStackTrace();
	...
	public void verifySorForStatus()
	{
		try
		{
			...
			waitIfElementClickIsIntercepted(dashboardObj.employees, "click", "");
			dashboardObj.nurseList.click();
			expectedMsg = verifySortButton(nurselistObj.statusSortButton, nurselistObj.sortedStatus);
			actualMsg = nurselistObj.sortedStatus.getText();
			waitIfElementClickIsIntercepted(dashboardObj.employees, "click", "");
		}
		catch(Exception e) {
			e.printStackTrace();
